...
public class NoteService : INoteService
{
    ...
    {
        var sql = "SELECT * FROM notes WHERE \"UserID\" = @UserId AND \"NoteTitle\" ILIKE @Title";          // ILIKE = case‑insensitive
    ...
    {
        var sql = "INSERT INTO notes (\"NoteTitle\", \"NoteContent\", \"UserID\", \"CreatedOn\")VALUES (@Title, @Content, @UserId, CURRENT_TIMESTAMP) RETURNING \"NoteID\";";
        return await db.ExecuteScalarAsync<int>(sql, note);  // new NoteID
    }

    public async Task<int> UpdateNote(Note note)
    {
        var sql = "UPDATE notes SET \"NoteTitle\" = @Title, \"NoteContent\" = @Content, \"UpdatedOn\" = CURRENT_TIMESTAMP WHERE \"NoteID\" = @NoteId AND \"UserID\" = @UserId;";
